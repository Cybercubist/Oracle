...
def plot_oracle(df, present_line, title, start_line):
    ...
    fig.update_layout(
        shapes = [dict(x0=present_line, x1=present_line, y0=0, y1=1, xref='x', yref='paper', line_width=2)],
        annotations=[dict(
        x=start_line, y=1, xref='x', yref='paper',
        showarrow=False, xanchor='left', text=title, font_size=14)]
    )
